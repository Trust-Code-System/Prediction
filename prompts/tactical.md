# Tactical Comparison contract

Mirrors `lib/tactical/prompt.ts`. Keep the two in sync.

Claude infers each side's likely tactical setup from cached Supabase data only
(squad by position, last-5 form, scoring patterns). There is no lineup feed, so
formation and style are a likely read, never confirmed fact. Output is validated
in code before storage; on total failure nothing is stored and the fixture is
retried on the next run. The match page hides the section when no row exists.

- Model: `claude-opus-4-8` primary, OpenAI fallback (same chain as predictions).
- Stored in `tactical_analysis` keyed by `fixture_id`.
- No betting language. No em dashes (the validator rejects any string containing one).
